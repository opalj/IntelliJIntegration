...
import org.opalj.intellijintegration.opalintegration.Visitor.StackMap.StackMapVisitorImpl;
import scala.Option;
...
final class Tables {
  ...
  @NotNull
  static String annotationsToJava(
  ...
  /**
   * E.g. if the method throws two exceptions, say IOException and RuntimeException, then the output
   * will be: throws java.io.IOException, java.lang.RuntimeException
   *
  ...
  static String innerClassTable(@NotNull ClassFile classFile) {
    ...
    classFile
        ...
        .foreach(
            innerClass -> {
              try {
                if (innerClass.outerClassType().isDefined() && innerClass.innerName().isDefined()) {
                  String innerClassEntry =
                      String.format(
                          "\t\t%s { %s %s }\n",
  ...
  static String localVariableTypeTable(@NotNull Code body) {
    ...
    localVariableTypeTable
        ...
        .append("// [size: ")
        .append(
            body.localVariableTypeTable()
                .size()) // TODO: not always correct? see Cl.ManyMethods#genericMethod
        .append(" item(s)]\n");
  ...
  static String methodTypeSignature(Method method) {
    ...
    methodTypeSig
        .append("\t// MethodTypeSignature: ")
        .append(methodTypeSignature.toJVMSignature())
        .append("\n");
            ...
            .map(
                e ->
                    "\t\ttry ["
                        ...
                        + e.handlerPC()
                        + " "
                        + (e.catchType().isDefined() ? e.catchType().get().toJava() : "ANY"))
