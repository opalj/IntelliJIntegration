...
import static org.opalj.intellijintegration.JavaByteCodeLanguage.autoGenerated.parser.JavaByteCodeParserUtil.*;
...
public class JavaByteCodeParser implements PsiParser, LightPsiParser {
  ...
  /* ********************************************************** */
  // ModifierV? (AT?) JAVATYPEHEAD JType (LBRACKET JType COLON RBRACKET JType COLON COLON (NUMBER)?
  // JType)? (EXTENDS JType)? (IMPLEMENTS (JType(COMMA)?)+)?
  ...
  // (LBRACKET JType COLON RBRACKET JType COLON COLON (NUMBER)? JType)?
  ...
  private static boolean ClassHead_4_0(PsiBuilder b, int l) {
    ...
    r = consumeToken(b, LBRACKET);
    r = r && JType(b, l + 1);
    r = r && consumeTokens(b, 0, COLON, RBRACKET);
    r = r && JType(b, l + 1);
    r = r && consumeTokens(b, 0, COLON, COLON);
    r = r && ClassHead_4_0_7(b, l + 1);
    r = r && JType(b, l + 1);
    exit_section_(b, m, null, r);
    return r;
  }
  ...
  // (NUMBER)?
  private static boolean ClassHead_4_0_7(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "ClassHead_4_0_7")) return false;
  ...
  /* ********************************************************** */
  // MNEMONIC (       LBRACKET JType COLON Type RBRACKET                       // e.g. PUT(someFQN :
  // someFQN)
  //                           | LBRACKET NUMBER RBRACKET                                 // e.g.
  // GOTO(5)
  //                           | LBRACKET STRING RBRACKET                                 // e.g.
  // LDC("someString")
  //                           | LBRACKET ((JavaOP|NUMBER|STRING)COMMA?)* RBRACKET        // e.g.
  // INVOKEVIRTUAL(byte[] { java.lang.Object clone() })
  //                           // TABLESWITCH
  ...
  /* ********************************************************** */
  // ((STRINGVAR | JbcTokens) DOT)*(STRINGVAR | JbcTokens) lambdaParams (STRINGVAR | JbcTokens)
  // COLON* NUMBER (STRINGVAR | JbcTokens)
