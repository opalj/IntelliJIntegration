...
public class Compiler {
        ...
        (aborted, errors, warnings, compileContext) -> {
          ...
          if (errors == 0) {
            VirtualFile classFile =
                getCorrespondingClassFile(compileContext.getProject(), virtualFile);
            ApplicationManager.getApplication()
                .invokeLater(
                    () ->
                        FileEditorManager.getInstance(compileContext.getProject())
                            .openFile(classFile, true),
                    ModalityState.NON_MODAL);
            FileEditorManager.getInstance(compileContext.getProject())
                .setSelectedEditor(classFile, editorName);
          } else {
  ...
  private VirtualFile getCorrespondingClassFile(Project project, VirtualFile javaFile) {
    ...
    if (Objects.requireNonNull(CompilerProjectExtension.getInstance(project)).getCompilerOutput()
        != null) {
      vfList.add(
          Objects.requireNonNull(CompilerProjectExtension.getInstance(project))
              .getCompilerOutput());
    }
    if (module != null) {
      if (Objects.requireNonNull(CompilerModuleExtension.getInstance(module))
